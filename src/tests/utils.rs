...
fn retrieve_asset() {
    ...
    // Local assets should not be included from data URL sources
    let (data, final_url) = utils::retrieve_asset(
        cache,
        &client,
        "data:text/html;base64,SoUrCe",
    ...
    // Local assets should not be included from remote sources
    let (data, final_url) = utils::retrieve_asset(
        cache,
        &client,
        "https://kernel.org/",
    ...
    // Local assets should be included from local sources
